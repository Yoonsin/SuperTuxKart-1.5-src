...
build_deps()
{
    ...
    if [ ! -f "$DIRNAME/deps-$ARCH_OPTION/curl.stamp" ]; then
        ...
        cmake . -G "Unix Makefiles" \
                ...
                -DMBEDTLS_INCLUDE_DIR="$DIRNAME/deps-$ARCH_OPTION/mbedtls/include/"              \
    ...
    if [ ! -f "$DIRNAME/deps-$ARCH_OPTION/shaderc.stamp" ]; then
        ...
        #cp -a -f "$NDK_PATH/sources/third_party/shaderc/libshaderc/"* "$DIRNAME/deps-$ARCH_OPTION/shaderc"
        ...
        #-DENABLE_GLSLANG_INSTALL=OFF, -DSPIRV_HEADERS_SKIP_INSTALL=1 -DENABLE_CTEST=0 
        cmake . -G "Unix Makefiles" \
                -DCMAKE_TOOLCHAIN_FILE=../../../cmake/Toolchain-android.cmake  \
        ...
        "$NDK_TOOLCHAIN_PATH/llvm-strip.exe" --strip-debug "$DIRNAME/deps-$ARCH_OPTION/shaderc/libshaderc/libshaderc_combined.a"
    ...
    if [ "$ARCH_OPTION" = "arm64-v8a" ]; then
        if [ ! -f "$DIRNAME/deps-$ARCH_OPTION/libadrenotools.stamp" ]; then
            ...
            #FIX Reference : https://github.com/supertuxkart/stk-code/pull/5770
        ...
        if [ ! -f "$DIRNAME/deps-$ARCH_OPTION/mesa.stamp" ]; then
            ...
            cp -R -L -f "$DIRNAME/../lib/mesa/"* "$DIRNAME/deps-$ARCH_OPTION/mesa"
...
[binaries]
#ar = ndk_path / 'toolchains/llvm/prebuilt/windows-x86_64/bin/llvm-ar.exe'
#nm = ndk_path / 'toolchains/llvm/prebuilt/windows-x86_64/bin/llvm-nm.exe'
#readelf = ndk_path / 'toolchains/llvm/prebuilt/windows-x86_64/bin/llvm-readelf.exe'
#objcopy = ndk_path / 'toolchains/llvm/prebuilt/windows-x86_64/bin/llvm-objcopy.exe'
ar = ndk_path / 'toolchains/llvm/prebuilt/windows-x86_64/bin/llvm-ar'
nm = ndk_path / 'toolchains/llvm/prebuilt/windows-x86_64/bin/llvm-nm'
readelf = ndk_path / 'toolchains/llvm/prebuilt/windows-x86_64/bin/llvm-readelf'
objcopy = ndk_path / 'toolchains/llvm/prebuilt/windows-x86_64/bin/llvm-objcopy'
# c = ['ccache', ndk_path / 'toolchains/llvm/prebuilt/windows-x86_64/bin/aarch64-linux-android26-clang']
# cpp = ['ccache', ndk_path / 'toolchains/llvm/prebuilt/windows-x86_64/bin/aarch64-linux-android26-clang++', '-fno-exceptions', '-fno-unwind-tables', '-fno-asynchronous-unwind-tables', '-static-libstdc++', '-Wno-c++11-narrowing']
c = ['ccache', ndk_path / 'toolchains/llvm/prebuilt/windows-x86_64/bin/aarch64-linux-android26-clang.cmd']
cpp = ['ccache', ndk_path / 'toolchains/llvm/prebuilt/windows-x86_64/bin/aarch64-linux-android26-clang++.cmd', '-fno-exceptions', '-fno-unwind-tables', '-fno-asynchronous-unwind-tables', '-static-libstdc++', '-Wno-c++11-narrowing']
c_ld = 'lld'
cpp_ld = 'lld'
...
strip = [ndk_path / 'toolchains/llvm/prebuilt/windows-x86_64/bin/llvm-strip.exe', '-s']
